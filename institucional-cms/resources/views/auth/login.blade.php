...
<head>
    ...
    <style>
        body {
            ...
            background: radial-gradient(circle at top, #E2E8F0 0%, #F3F4F6 48%, #F8FAFC 100%);
        }
        .card {
            width: min(420px, 92%);
            background: #fff;
...
<div class="card">
    ...
    @if(session('status'))
        <div class="error" style="background:#DCFCE7;border-color:#86EFAC;color:#166534;">{{ session('status') }}</div>
    @endif
    ...
    <form action="{{ route('login.attempt') }}" method="POST">
        ...
        <label class="row" for="remember" style="font-weight: normal;">
    ...
    <a class="back" href="{{ route('password.request') }}">Olvidé mi contraseña</a>

    <a class="back" href="{{ route('home') }}">Volver al sitio</a>
</div>
</body>
</html>
